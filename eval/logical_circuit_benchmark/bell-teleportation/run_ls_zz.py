"""
Bell State Teleportation — ZZ lattice surgery (vertical patch stack).

Two sequential ZZ merges: first patch2–patch3 (Bell prep), then patch1–patch2
...
Usage:
    ...
    python run_ls_zz.py -d 3 5 7 -p 5e-4 1e-3 2e-3 5e-3 --states X Z
"""
import argparse
import csv
import os
...
def build_circuit(d: int, teleport_state: str):
    """
    ZZ-LS Bell teleportation. Layout: patch1 (bottom), patch2, patch3 (top), step = 2d.
    """
    rounds_pre = d
    rounds_ls = d
    gap = 1
    ...
    init_dict = {
        q: init_zz[system.index_to_owner_map[q]]
    ...
    builder.initialize(
        init_dict={q: 'X' for q in cp23_global},
        n=system.num_qubits,
    )
    se_block = UnrotatedSurfaceCodeExtractionBlock(system)
    ...
    builder.initialize(
        init_dict={q: 'X' for q in cp12_global},
        n=system.num_qubits,
    )
    se_block = UnrotatedSurfaceCodeExtractionBlock(system)
    builder.apply_syndrome_extraction(circuit_chunk=se_block.circuit, rounds=rounds_ls)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='ZZ-LS Bell teleportation sweep — CPU PyMatching')
    ...
    csv_path = os.path.join(out_dir, 'ls_zz_results.csv')
    ...
    for d in args.distances:
        for state in args.states:
            print(f'\n{"="*60}')
            print(f'Building d={d}, state=|{state}⟩ (ZZ-LS)')
            print(f'{"="*60}')
