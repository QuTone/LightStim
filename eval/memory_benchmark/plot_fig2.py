...
from src.plot.styles import apply_paper_style, bold_ticks, CODES as _CODE_COLORS, CODE_LABELS
...
def load_data():
    """Load per-code CSVs and merge; falls back to combined fig2_bb_codes.csv."""
    dfs = []
    for code in FIG2_CODES:
        f = RESULTS / f"fig2_bb_codes_{code}_gpu_bposd.csv"
        if f.exists():
            df = pd.read_csv(f)
            df["decoder_label"] = "gpu_bposd"
            dfs.append(df)

    # Also load mwpf data if present in the combined CSV
    combined = RESULTS / "fig2_bb_codes.csv"
    if combined.exists():
    ...
    if not dfs:
        raise FileNotFoundError(
            "No fig2 CSV data found. Run: "
            "venv/bin/python -m eval.memory_benchmark.run_all --figure 2"
        )
