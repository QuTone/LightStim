...
@pytest.mark.parametrize("block_class", [GenericCSSGaugeExtractionBlock, SHYPSCodeExtractionBlock])
def test_extraction_measures_declared_gauges_and_preserves_bare_logicals(r, block_class):
    system = QECSystem()
    system.add_patch(SHYPSCode(r), name="shyps")
    extraction = block_class(system)
...
@pytest.mark.parametrize("r", [True, 2, 6, 3.5, "3"])
...
def test_public_shyps_memory_tracks_all_nine_logicals(basis):
    ...
    assert experiment.block_class is SHYPSCodeExtractionBlock
