"""
Bell State XX Teleportation — varying coupler_23 gap as proxy for routing distance.

Symmetric counterpart to run_ls_zz_dist.py:
  ZZ-LS: coupler in X basis → Z errors silent → Teleport |Z⟩ grows linearly with r
  XX-LS: coupler in Z basis → X errors silent → Teleport |X⟩ grows linearly with r

The Bell pair is prepared between patch2 and patch3.  When those two patches are
physically far apart, coupler_23 spans more qubits.  coupler_12 (source → Bell
anchor) always stays 1× adjacent.

Horizontal patch layout (x-direction), XX interaction.
...
Sub-experiments: teleport state |X⟩ and |Z⟩.
...
def build_circuit(d: int, teleport_state: str, routing_mult: int):
    """
    XX-LS Bell teleportation with long-range coupler_23 (Bell pair preparation).

    Horizontal layout: patch1(0,0), patch2(step_1x,0), patch3(step_1x + routing_mult*step_1x, 0).
    coupler_23 spans routing_mult × step_1x (Bell pair — long distance).
    ...
    Basis symmetry vs ZZ-LS:
        ZZ-LS: patches/coupler initialised in X (|+⟩), measured X
        XX-LS: patches/coupler initialised in Z (|0⟩), measured Z
    """
    rounds_pre = d
    rounds_ls  = d
    gap     = 1
    ...
    # patch2 always 1× from patch1 (horizontal)
    system.add_patch(patch2_local, name='patch2', offset=(step_1x, 0))
    # patch3 is routing_mult × step_1x from patch2 (horizontal)
    system.add_patch(patch3_local, name='patch3', offset=(step_1x + routing_mult * step_1x, 0))
    ...
    # Initialise: patch1 = teleport_state, patch2/3 = |0⟩ (Z eigenstates)
    init_map = {'patch1': teleport_state, 'patch2': 'Z', 'patch3': 'Z'}
    init_dict = {
    ...
    builder.initialize(init_dict={q: 'Z' for q in cp23_global}, n=system.num_qubits)
    se_block = UnrotatedSurfaceCodeExtractionBlock(system)
    builder.apply_syndrome_extraction(circuit_chunk=se_block.circuit, rounds=rounds_ls)
    builder.deactivate_coupler('coupler_23')
    # Measure coupler_23 data qubits immediately at deactivation (Z basis for XX-LS)
    ...
    builder.initialize(init_dict={q: 'Z' for q in cp12_global}, n=system.num_qubits)
    se_block = UnrotatedSurfaceCodeExtractionBlock(system)
    builder.apply_syndrome_extraction(circuit_chunk=se_block.circuit, rounds=rounds_ls)

    # Final readout: patch1→Z (discard), patch2→Z (discard), patch3→teleport_state
    meas_map = {'patch1': 'Z', 'patch2': 'Z', 'patch3': teleport_state}
    meas_dict = {
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Bell XX-LS teleportation — varying coupler_23 routing distance')
    ...
    csv_path = os.path.join(out_dir, 'ls_xx_dist_results.csv')
    ...
    for d in args.distances:
        for state in args.states:
            for mult in args.mults:
                ...
                print(f'\n{"="*60}')
                print(f'Building {label}  (XX-LS, coupler_23 gap={mult}×step, horizontal)')
                print(f'{"="*60}')
