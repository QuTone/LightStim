...
def _resolve(filename: str) -> Path | None:
    """Return path if file exists in results/ or precomputed/, else None."""
    for d in (RESULTS, PRECOMPUTED):
        p = d / filename
        if p.exists():
            return p
    return None
...
def _glob_both(pattern: str) -> list[Path]:
    """Glob pattern across results/ first, then precomputed/ for missing files."""
    seen = set()
    paths = []
    for d in (RESULTS, PRECOMPUTED):
...
SC_STYLE = {
    "rotated_sc":   dict(color="#2166ac", ls="-",   marker="o", label="Rotated SC"),
    "unrotated_sc": dict(color="#4dac26", ls="--",  marker="s", label="Unrotated SC"),
    "toric":        dict(color="#d01c8b", ls=":",   marker="^", label="Toric"),
    "color":        dict(color="#e6550d", ls="-.",  marker="D", label="Color (6-6-6)"),
}

# ── BB / 4D code style ────────────────────────────────────────────────
BB_STYLE = {
    "bb_72_12_6":      dict(color="#a63603", marker="*", label="[[72, 12, 6]]"),
    "bb_108_8_10":     dict(color="#1b9e77", marker="*", label="[[108, 8, 10]]"),
    "bb_144_12_12":    dict(color="#7570b3", marker="*", label="[[144, 12, 12]]"),
...
def load_all():
    ...
    # Surface codes (fig1)
    f = _resolve("fig1_surface_codes.csv")
    if f is not None:
        df = pd.read_csv(f)
    ...
    for f in _glob_both("fig2_bb_codes_*_gpu_bposd.csv"):
        df = pd.read_csv(f)
        df = df[np.isclose(df["p"], 1e-3)].copy()
        if df.empty:
            continue
    ...
    # Color code (fig3) — check results/fig3_color_code.csv first, then precomputed/
    for fname in ("fig3_color_code.csv",):
        f = _resolve(fname)
        if f is not None:
            df = pd.read_csv(f)
            df = df[np.isclose(df["p"], 1e-3)].copy()
    ...
    # 4D Hadamard
    f = _resolve("fig3_4d_hadamard.csv")
    if f is not None:
        df = pd.read_csv(f)
        dfs.append(df)
...
def plot(df):
    ...
    for code_name, style in BB_STYLE.items():
        ...
        ax.semilogy([x_val], [y_val],
                    ...
                    markersize=16, markeredgecolor="none",
    ...
    # ── Axes ──────────────────────────────────────────────────────────
    ax.set_xlabel(r"Physical Qubits per LogQ ($n/k$)")
    ax.set_ylabel("LER per Round per LogQ")
    ax.set_title(r"Code Comparison ($p = 10^{-3}$)")
    ax.set_xlim(left=0)
