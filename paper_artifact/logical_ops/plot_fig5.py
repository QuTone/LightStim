"""
Regenerate fig5_s.png — S gate logical error rate (S_oneway).

Uses S_oneway LER directly (no aggregation needed), color by distance.
...
def plot(df):
    ...
    # S_oneway: use LER directly (single sub-experiment per d, p)
    df_s = df[df["sub_experiment"] == "S_oneway"].copy()

    dist_proxy = []
    for d in sorted(df_s["d"].unique()):
        df_d = df_s[df_s["d"] == d].sort_values("p")
    ...
    ax.set_title("S Gate (S$_{\\mathrm{oneway}}$)")
