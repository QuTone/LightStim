...
# Explicit circulants for the three supported SHYPS sizes.
# r=3 uses the polynomial in Example VIII.7; r=4 uses another primitive
# trinomial in the construction of Section VIII.4; r=5 uses 1+x^2+x^5.
# All three classical kernels
# are checked exhaustively against the simplex weight distribution in tests.
_SIMPLEX_CHECK_EXPONENTS = {3: (0, 2, 3), 4: (0, 1, 4), 5: (0, 2, 5)}


class SHYPSCode(QECPatch):
    """A CSS subsystem hypergraph-product simplex code.

    ``r=3`` gives ``[[49, 9, 4]]``, ``r=4`` gives ``[[225, 16, 8]]``, and
    ``r=5`` gives ``[[961, 25, 16]]``;
    ...
    * their centre ``S_X = H kron C`` and ``S_Z = C kron H``.
    ...
    The default extraction uses three fixed-offset CNOT layers per basis,
    measuring X gauges then Z gauges. Its
    ...
    Args:
        r: Simplex dimension, currently 3, 4 or 5.
    ...
    def _process_params(self) -> None:
        ...
        if isinstance(raw_r, bool) or not isinstance(raw_r, Integral):
            raise ValueError("r must be an integer, either 3, 4 or 5.")
        self.r = int(raw_r)
        if self.r not in _SIMPLEX_CHECK_EXPONENTS:
            raise ValueError("SHYPSCode currently supports r=3, r=4 and r=5.")
    ...
    @property
    def gauge_offsets(self) -> tuple[int, int, int]:
        """Cyclic check supports, also the default CNOT-layer order."""
        return _SIMPLEX_CHECK_EXPONENTS[self.r]

    @property
    def default_extraction_block_class(self):
        from .SE_block import SHYPSCodeExtractionBlock

        return SHYPSCodeExtractionBlock

    def build(self) -> None:
        m = self.simplex_length
        first_row = np.zeros(m, dtype=np.uint8)
        first_row[list(self.gauge_offsets)] = 1
    ...
    def get_info(self):
        ...
        info.update(
            r=self.r,
            gauge_offsets=self.gauge_offsets,
